...
Select * From Customer 
WHERE CustomerID BETWEEN (SELECT CustomerID 
FROM CustomerOrder WHERE OrderDate = '2019-01-13')
AND 5;
...
Select * From Customer 
WHERE CustomerID >SOME(SELECT CustomerID 
FROM CustomerOrder WHERE Bill>10);
...
SELECT ORDERID FROM CustomerOrder WHERE SalesmanId IN (SELECT SalesmanId FROM Salesman WHERE Salary > (
